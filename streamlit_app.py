...
# === NEW IMPORTS FOR PDF (additive; does not affect existing features) ===
...
def build_pdf_report_standard(cells_ll, merged_ll, user_inputs, cell_size, overlay_present):
    ...
    pdf.multi_cell(0, 5, "Note: Areas computed using UTM for better accuracy. "
...
# === NEW: optional PDF toggle (doesn't change existing flow) ===
...
if st.session_state["generated"]:
    ...
    if uploaded_aoi is not None:
        ...
        # ⚠️ IMPORTANT: different generators for different buttons
        ...
        # === NEW: PDF download (only added, no change to existing flow) ===
        with col3:
            if generate_pdf:
